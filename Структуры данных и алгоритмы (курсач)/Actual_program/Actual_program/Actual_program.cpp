...
bool lee_upgrade(int ax, int ay, int bx, int by)   // поиск пути из ячейки (ax, ay) в ячейку (bx, by)
{
			...
			for (x = 0; x < W; ++x)
				if (object_matrix[x][y] == d)                         // ячейка (x, y) помечена числом d
				{
					...
					{  
						...
						while (object_matrix[ix][iy] != WALL && iy >= 0 && iy < H && ix >= 0 && ix < W)
						{
							if (iy >= 0 && iy < H && ix >= 0 && ix < W && object_matrix[ix][iy] == BLANK && object_matrix[ix][iy] != WALL)
							{
	...
	while (d > 0)
	{
		...
		for (k = 0; k < 4 && !stop; ++k)
		{
			...
			while (iy >= 0 && iy < H && ix >= 0 && ix < W && object_matrix[ix][iy] > 0)
			{
				...
				if (object_matrix[ix][iy] == d && ix < 8 && iy < 8)
				{
					stop = true;
					x = ix;
	...
	px[0] = ay;
	py[0] = ax;                    // теперь px[0..len] и py[0..len] - координаты ячеек пути
