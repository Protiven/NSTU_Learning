...
<div class="">
	<div class="" style="display: table-cell">
		<h3><?=$row['title']?></h3>
		<div class="" ><img class="" style="width:100%;" src="<?=$row['addr_im']?>" ></img></div>
		<span>
			<?=$row['full_text']?>
		</span>
		<?php if(isset($_SESSION['logged_user']) && isset($_SESSION['FLAG_ADM']) && $_SESSION['FLAG_ADM'] == true){?>
		<a class="" style="font-family: Arial; font-weight: 500" href="change_new.php?id=<?=$row1?>"> (Изменить публикацию)</a>
		<?php }?>
		<div class=""><?=$row['date']?> <?=$row['time']?></div>
	</div>
</div>
